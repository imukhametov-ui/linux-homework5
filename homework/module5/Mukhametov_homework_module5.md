...
Коментар:
Локальна IP-адреса: 10.0.2.15

Доступ до інтернету: є (ping 8.8.8.8 виконується успішно, втрат пакетів немає — 0%).

Приклад сервісу, що слухає порт: ssh (порт 22).
...
Локальний файл test.txt був переданий на сервер через scp. Для синхронізації створено директорію /home/ubuntu/sync_dir та використано rsync. Наявність файлу перевірено через sftp командою ls.
